    ...
    if st.session_state['usercheck']==True:
        if st.session_state["st"]==True:
            ...
            if st.button("Confirm"):
                ...
                st.write(dx)
                sql = """INSERT INTO qst1 (Username, Nome, Cognome,Livello_sodisfazione, q1, q2,q3 ,q4 ,q5 , time) VALUES (pd.DataFrame(get_data())["Username”].loc[-1],pd.DataFrame(get_data())[“Nome”].loc[-1],pd.DataFrame(get_data())[“Cognome”].loc[-1],pd.DataFrame(get_data())[“Livello_sodisfazione”].loc[-1],pd.DataFrame(get_data())[“q1”].loc[-1],pd.DataFrame(get_data())[“q2”].loc[-1],pd.DataFrame(get_data())[“q3”].loc[-1],pd.DataFrame(get_data())[“q4”].loc[-1],pd.DataFrame(get_data())[“q5”].loc[-1],pd.DataFrame(get_data())[“time"].loc[-1])"""
                cursor = conn.cursor()
                cursor.execute(sql)
